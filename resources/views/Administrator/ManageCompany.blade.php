...
<div class="navbar">
    ...
    <a href="{{url('')}}/administrator/company/manage" style="color: white; background-color: red; font-weight: bolder; border: 2px solid white; border-radius: 29.9px;">Manage Companies</a>
...
<main>
    ...
    <center>
        ...
        @enderror
            <h4 style='color: red'>Company : <span style='color: green;'>{{$CompanyId}}</span> is Deleted Succesfully.</h4>
        @endif

        @if (isset($cinnotfoundindatabase))
        @error('CIN')
            <h4 style='color: red'>{{$message}}</h4>
        @enderror
            <h4 style='color: red'>Company : <span style='color: green;'>{{$CompanyId}}</span> is not found in our database.</h4>
        @endif
        @if (isset($deleteindustry))
        @error('Industry')
            <h4 style='color: red'>{{$message}}</h4>
        @enderror
            <h4 style='color: red'>Industry : <span style='color: green;'>{{$Industry}}</span> is Deleted Succesfully.</h4>
        @endif
        @if (isset($industrynotfoundindatabase))
        @error('Industry')
            <h4 style='color: red'>{{$message}}</h4>
        @enderror
            <h4 style='color: red'>Industry : <span style='color: green;'>{{$Industry}}</span> is not found in our database.</h4>
        @endif
        <div id="form">
            <br><br>
            <h4 style="color: white; background-color: red; display: inline; padding: 4.9px; border-radius: 29.9px;">Delete a Company</h4><br><br>
        <form action="{{url('')}}/administrator/company/manage/cin" Method='POST'>
            <center>
                @csrf
        <input type='Text' placeholder='Enter CIN *' id="Endorsement _No" name="CIN" style="width: 65%; height: 19.9px;"> &nbsp; &nbsp;
        <input type='submit' value='Delete' id="solved" name="Delete">
           </form>
           <br><br>
           <h4 style="color: white; background-color: red; display: inline; padding: 4.9px; border-radius: 29.9px;">Delete Whole Industry</h4>
           <br><br>
        <form action="{{url('')}}/administrator/company/manage/industry" Method='POST'>
            @csrf
            <center>
        <Select name="industry" style="width: 65%; height: 29.8px;">
            <option Selected>Select Industry</option> 
            <option value="IT">Information Technology</option>
            <option value="Business">Business</option>
            </Select>
            &nbsp; &nbsp;
        <input type='submit' value='Delete' id="solved" name="DeleteIndustry">
        <br><br>
           </form>
           </div>
           </center>
...
@if (isset($CompanyId) || isset($deletecin) || isset($cinnotfoundindatabase))
