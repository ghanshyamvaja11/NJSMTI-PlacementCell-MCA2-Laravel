...
<div class="navbar">
    ...
    <a href="{{url('')}}/student/applies" style="color: white; background-color: red; font-weight: bolder; border: 2px solid white; border-radius: 29.9px;">Applies</a>
...
<main>
<center> <h1 style="color: white; background: blue; border-radius: 29px; font-size: 24.9px;">Job Applies</h1></center>

@if (isset($ApplicationExists))
           <center><h4 style='color: red'>{{$ApplicationExists}}</h4></center>
    @endif
    @if(isset($count) and count($applies) == 0)
        ...
        <table>
          ...
          <tr>
            <th>{{$job->JobId}}</th>
                ...
                @if (isset($Status))
                    @if ($Status == "Y")
                        <th style="color: rgb(5, 212, 5);">seen</th>
                    @elseif($Status == "N")
