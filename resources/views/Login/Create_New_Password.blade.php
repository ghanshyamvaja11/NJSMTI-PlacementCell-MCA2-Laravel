...
<link rel="icon" type="image/x-icon" href="{{asset('Images/Header&Footer/favicon.jpg')}}">
    ...
    <link rel="stylesheet" href="{{asset('css/Create_New_Password.css')}}">
...
<body>
@if(isset($UserType) and isset($Email))
    <header id="header">
        <center>
...
<main>
    ...
    @endif
    @error('password')
                <center><h4 style="color: red; fonnt-size: 15.9px;">{{$message}}</h4></center>
            @enderror
    <section>
        <p>
        <center>
        <form action="{{url('')}}/forgotpassword/keyvarification/createnewpassword" method="POST">
            @csrf
            <fieldset>
            <legend>Create New Password</legend>
<center><img src="{{asset('Images/Login/Logged_in.jpg')}}" style='background-color: white; height: 96px; width: 95px; border-radius: 59px;'><br>
@if($UserType != ("Student" && "Company"))
<h4 style='color: white;'>
{{$UserType}}
</h4>
@else
    <h4 style='color: white;'> {{$UserType}} </h4>
@endif
</center>
<center>
                <p>
            <input type="Password" name="password" id="New_Password" placeholder="Enter New Password *" required><br><br>
            <input type="Password" name="password_confirmation" id="Confirm_Password" placeholder="Confirm New Password *" required></center><br>
...
function Speech(){
  ...
  message.text="Create new Password";
